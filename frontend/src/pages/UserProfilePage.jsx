...
export default function UserProfilePage() {
  ...
  async function load() {
    try {
      ...
      setUser(u); setPrompts(p); setWorkspaces(w); setStarred(s)
  ...
  function openEdit() {
    setForm({ avatarUrl: user.avatarUrl || '', bio: user.bio || '' })
    setShowEdit(true)
  }
  ...
  if (!user) return <div className="page"><div className="empty-state">User not found.</div></div>
      ...
      <div className="page">
        ...
        <div className="profile-header">
          <div className="profile-avatar-wrap">
            <div className="profile-avatar">
              <img
                  src={user.avatarUrl || `https://api.dicebear.com/9.x/initials/svg?seed=${user.username}&backgroundColor=0d3330&textColor=2dd4bf&fontSize=40`}
                  alt={user.username}
              />
            </div>
          ...
          {isOwner && (
              <button className="btn btn-secondary profile-edit-btn" onClick={openEdit}>Edit Profile</button>
          )}
        </div>
        ...
        {tab === 'workspaces' ? (
            <div className="ws-grid">
              ...
              {workspaces.length === 0 && <div className="empty-state">No workspaces yet.</div>}
            </div>
        ) : (
            <div className="prompts-grid">
              {tabContent[tab].map(p => <PromptCard key={p.id} prompt={p} onStarChange={load} />)}
              {tabContent[tab].length === 0 && (
                  <div className="empty-state">
                    {tab === 'starred' ? 'No starred prompts yet.' : 'No prompts yet.'}
                  </div>
              )}
            </div>
        )}
            ...
            <Modal title="Edit Profile" onClose={() => setShowEdit(false)}>
              <form onSubmit={handleEdit}>
                <div className="form-group">
                  ...
                  <input className="form-input" value={form.avatarUrl} onChange={e => setForm(f => ({ ...f, avatarUrl: e.target.value }))} placeholder="https://..." />
                </div>
                <div className="form-group">
                  <label className="form-label">Bio</label>
                  <textarea className="form-textarea" value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} placeholder="Tell the community about yourself" />
                </div>
